...
function PokemonCard({ pokemon }) {
  ...
  return (
    <button
      className={`${styles["card-background"]} ${styles["effect3D"]} flex flex-row items-center justify-between md:py-2 hover:-translate-y-4 transition-transform duration-300 ease-in-out cursor-grab h-full`}
      onMouseDown={handleMouseDown}
      onClick={handleClick}
    >
      <div
        className={`border-y-4 md:border-y-4 lg:border-y-8 ${styles[generation]} overflow-y-hidden p-1 h-44 md:h-full lg:h-full flex flex-col items-center justify-between gap-2 truncate`}
      >
